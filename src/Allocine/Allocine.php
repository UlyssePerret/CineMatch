...
class Allocine
{
    ...
    public function search($query, $filter = 'movie', $page = 1)
    {
        // build the params
        $params = array(
    ...
    public function getSessionMovie($zip = null, $movie = null, $date = null, $radius = null, $page = 1)
    {
        $params = array(
            'partner' => $this->_partner_key,
            'format' => 'json',
        );
        foreach (['zip', 'movie', 'date', 'radius', 'page'] as $f) {
            if ($$f !== null) {
                $params[$f] = $$f;
            }
        }

        return $this->_do_request('showtimelist', $params);
    }
}
